...
const CarrierCard = (props) => {
    ...
    useEffect(() => {
        ...
        // console.log(booking.init_line_bookmark_seq, booking.line_bookmark_seq)
        if( booking.init_line_bookmark_seq != booking.line_bookmark_seq ) {
            // Line Bookmark로 booking의 Line 입력하기
            updateLineOfBooking();
        }
    },[booking]);
    ...
    useEffect(()=>{
        if( props.booking.bkg_no ) {
            ...
            // Line Bookmark seq
            let line_bookmark_seq = props.booking.line_bookmark_seq;
            let init_line_bookmark_seq = props.booking.init_line_bookmark_seq;

            setBooking({...booking, 'bkg_no':bkg_no, 'bkg_date':bkg_date
                , 'status_cus':status_cus, 'sc_no':sc_no, 'user_no':user_no
                , 'line_bookmark_seq':line_bookmark_seq
                , 'init_line_bookmark_seq':init_line_bookmark_seq
            }); // 초기화 bookmark seq 값

            // 최초 조회하기
            selectLineOfBooking(props.booking);
        }
    },[props.booking]);
    ...
    const fncSelectLine=(e)=>{
        ...
        // 그외 데이터인 경우
            ...
            if( e.target.value == element.line_bookmark_seq) {
                // console.log(element)
    ...
    const updateLineOfBooking = () => {
        axios.post(
            ...
            res => {
                setBooking({...booking, 'init_line_bookmark_seq':booking.line_bookmark_seq});
            ...
            <Col xl="12" lg="12">
                <Card>
                    ...
                    <CardBody className="pt-3 pb-2" style={{border:'1px solid silver',borderRadius:'10px'}}>
                        <Row className="pb-4">
                            <Col className="mt-2 mb-0 col-5" style={{fontSize:'15px',color:'#696969',fontWeight:'600'}}>CARRIER</Col>
                            <Col>
                                <Input type="select"
                                    ...
                                    value={booking.line_bookmark_seq?booking.line_bookmark_seq:'0'}>
                                    <option key={0} value={'0'}>
                                        선택
                                    </option>
                                    {lineList.length > 0 ?lineList.map((element,key)=>{
                                        return(
                                            <option key={key} value={element.line_bookmark_seq}>
                                                {element.line_bookmark_name}
                                            </option>
                                        )
                                    })
                                    :<></>}
