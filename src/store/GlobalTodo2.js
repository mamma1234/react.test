...
const GlobalTodo2 = observer(({store,...props}) => {    
// const GlobalTodo2 = observer((store, ...props) => {    
  ...
  const setMobx = ( ) => {
    ...
    store.setting('todomobx_token', {username:'global2', usertype:'C'});
    gstore.setting('todomobx_token', {username:'global2', usertype:'C'});
  ...
  const getMobx = ( ) => {
    ...
    setName2(store.getusername);
    setGname(gstore.username);
    setGname2(gstore.getusername);
  }  
        ...
        <CardBody className="pt-2 pb-2 bg-white">
            <Row>
                <Col className="ml-auto mr-auto" xl="10" lg="10" md="10" sm="10">
                    ...
                    <h4>GlobalStore.action->Stats:{name2}</h4>
                    <h4>ContextStore:{gstore.username}</h4>
                    <h4>ContextStore->Stats:{gname}</h4>
                    <h4>ContextStore.action->Stats:{gname2}</h4>
